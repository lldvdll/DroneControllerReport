...
We implement REINFORCE \cite{CourseNotes}, a policy gradient method that directly optimises a stochastic policy $\pi_{\theta}(a|s)$. The parameters $\theta$ are updated via gradient ascent on the expected return $J(\theta)$, approximated by sampling trajectories:
\begin{equation}
...
We begin with a hover curriculum, with one permanent target at (0,0). Rewards are shaped to encourage target proximity and penalise instability. In order to overcome the return smoothing of REINFORCE, we reward intention rather than outcome and localise returns with a low discount factor (0.5). Introducing a positive radius to a distance-to-target reward creates a basin of attraction around the target. We penalise actions which amplify drift, using asymmetry to avoid catastrophic oscillation from overcorrection. $r_{drift} = \min(0,-v_x \cdot \phi)$. We note that \textbf{Actor-Critic} (AC) \cite{CourseNotes} is a more principled approach to the same objective and should be investigated in future work.

Training diagnostic plots (\Cref{fig:hover_success}) show that monotonic increases in both cumulative rewards and survival time are correlated with gradual exploration decay and decrease in gradient variance. As the network settles into a consistent parameterisation, thrust and roll outputs converge with decreasing variance to counteract gravity. When tested, the agent approaches targets, acquiring them with long enough episode length. The agent learned to stabilise temporarily near targets.

In the subsequent target seeking phase, the agent failed to navigate consistently to more than one target. Both \textbf{Weight Decay} to escape the hover extrema, and a \textbf{Value Baseline} using a target seeking SARSA model, were unsuccessful as training failed to stabilise. The agent learned stable hover behaviour, however learning to navigate from here would require either significant gradual unlearning, or better variance management.

\begin{figure}
    \centering
    \includegraphics[width=\singlefigure]{figures/neural_reinforce_hover_success.png}
    \caption{\label{fig:hover_success} Training diagnostics. TL: cumulative rewards (blue), survival time (yellow). TR: sampled actions. BL: average sigma (exploration rate). BR: gradient norms.}
\end{figure}