...
REINFORCE is a policy gradient method which directly learns a stochastic policy $\pi_{\theta}(a|s)$. Discounted returns $G_t$ are caluclated over a full episode of simulation, parameters $\theta$ of the policy model are updated such that the log probability of each action $a_t$ given state $s_t$ will increase or decrease in proportion to the return $G_t$. 
